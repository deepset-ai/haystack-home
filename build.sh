...
git clone -b bilge-walkthroughs --filter=tree:0 https://github.com/deepset-ai/haystack-tutorials.git
...
if [[ "$DEPLOY_URL" != "localhost" ]]; then
    ...
    find ./content/guides -name "index.md" -type f -exec bash -c '
    dir=$(dirname "{}" | sed -e "s,^.*content/guides/,," -e "s,/.*,,");
    sed -i "/\(http\|\/images\)/! s~!\[\([^]]*\)\]([./]*\([^)]*\))~![\1]($dir/\2)~g" "{}"
    ' \;
fi
