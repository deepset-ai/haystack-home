...
I'm going to create a similar component for Haystack 2.0, which we will call `HallucinationChecker`. This component gets both the `statement` generated by the LLM and the `documents` used by the LLM to generate the statement. After looking at `statement` and `documents`, it tries to infer whether the generated statement is supported or contradicted by the documents. If you're interested in the exact implementation of this component, you can check out the [tutorial on `HallucinationChecker`](https://colab.research.google.com/drive/1cbHRSSrkadTv-prhnFiG5d9d9RDPCIqu#scrollTo=d7CiPoKUPwuI), or if you're interested in creating your custom component, check out [Creating Custom Components](https://docs.haystack.deepset.ai/v2.0/docs/custom-components) guide.
